...
const { close } = defineProps<{ close: CloseHandler<TitleAction> }>();

const selections: [ string, () => any ][] = [
    [ "开始游戏", () => {
        close([ TitleOperator.START_NEW , ""]);
    } ],
    [ "读取存档", async () => {
        const save = await showLoad();
        if (save === null) return;
        close([ TitleOperator.START_FROM_SAVE, "" ]);
    } ],
    [ "录像回放", async () => {
        try {
             ...
             * @todo 解码route
             */
            close([ TitleOperator.START_FROM_REPLAY , route]);
        } catch {
