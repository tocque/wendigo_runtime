...
export class Enemys {
    ...
    getDamageString(enemy, x, y, floorId) {
        ...
        if (damage == null) {
            ...
            if (damage <= 0)
                color = '#11FF11';
            else if (damage < core.status.hero.hp / 3)
                color = '#FFFFFF';
            else if (damage < core.status.hero.hp * 2 / 3)
                color = '#FFFF00';
            else if (damage < core.status.hero.hp)
                color = '#FF9933';
            else
                color = '#FF2222';
    ...
    _nextCriticals_useLoop(enemy, info, number, x, y, floorId) {
        ...
        for (var atk = start_atk + 1; atk <= mon_hp + mon_def; atk++) {
            ...
            if (nextInfo == null || (typeof nextInfo == 'number'))
                break;
            if (pre > nextInfo.damage) {
                pre = nextInfo.damage;
                list.push([atk - hero_atk, info.damage - nextInfo.damage]);
                if (nextInfo.damage <= 0 && !core.flags.enableNegativeDamage)
                    break;
    ...
    _nextCriticals_useBinarySearch(enemy, info, number, x, y, floorId) {
        ...
        while (true) {
            ...
            if (pre <= 0 && !core.flags.enableNegativeDamage)
                break;
            if (list.length >= number)
                break;
        }
    ...
    getDefDamage(enemy, k, x, y, floorId) {
        ...
        if (nowDamage == null || nextDamage == null)
            return "???";
    ...
    _getDamage(enemy, hero, x, y, floorId) {
        if (enemy == null)
            enemy = core.getBlockId(x, y, floorId);
        if (typeof enemy == 'string')
            enemy = core.material.enemys[enemy];
        if (enemy == null)
            return null;
    ...
    hasEnemyLeft(enemyId, floorId) {
        ...
        if (enemyId instanceof Array)
            enemyId.forEach(function (v) { enemyMap[v] = true; });
        else if (enemyId)
            enemyMap[enemyId] = true;
        else
            enemyMap = null;
        for (var i = 0; i < floorId.length; i++) {
            ...
            for (var b = 0; b < mapBlocks.length; b++) {
                if (!mapBlocks[b].disable && mapBlocks[b].event.cls.indexOf('enemy') === 0) {
                    if (enemyMap === null || enemyMap[core.getFaceDownId(mapBlocks[b])])
                        return true;
                }
            }
        }
