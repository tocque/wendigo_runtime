...
export class UI {
    ...
    setFilter(name, filter) {
        ...
        if (!filter)
            ...
            var x = [];
            if (filter.blur > 0)
                x.push('blur(' + filter.blur + 'px)');
            if (filter.hue > 0)
                x.push('hue-rotate(' + filter.hue + 'deg)');
            if (filter.grayscale > 0)
                x.push('grayscale(' + filter.grayscale + ')');
            if (filter.invert)
                x.push('invert(1)');
            if (filter.shadow > 0)
    ...
    setFillStyle(name, style) {
        var ctx = this.getContextByName(name);
        if (ctx)
            ctx.fillStyle = core.arrayToRGBA(style);
    }
    ////// 设置某个canvas边框属性 //////
    setStrokeStyle(name, style) {
        var ctx = this.getContextByName(name);
        if (ctx)
            ctx.strokeStyle = core.arrayToRGBA(style);
    }
    ////// 设置某个canvas的对齐 //////
    ...
    _uievent_setAttribute(data) {
        this._createUIEvent();
        if (data.font)
            this.setFont('uievent', data.font);
        if (data.lineWidth)
            this.setLineWidth('uievent', data.lineWidth);
        if (data.alpha != null)
            this.setAlpha('uievent', data.alpha);
        if (data.fillStyle)
            this.setFillStyle('uievent', data.fillStyle);
        if (data.strokeStyle)
            this.setStrokeStyle('uievent', data.strokeStyle);
        if (data.align)
            this.setTextAlign('uievent', data.align);
        if (data.baseline)
            this.setTextBaseline('uievent', data.baseline);
    ...
    drawImage(name: CtxRefer,
        image: CanvasImageSource | string, x: number, y: number): void
    drawImage(name: CtxRefer,
        image: CanvasImageSource | string, x: number, y: number, w: number, h: number): void
    drawImage(name: CtxRefer,
        image: CanvasImageSource | string,
        x: number, y: number, w: number, h: number, x1: number, y1: number, w1: number, h1: number): void
    drawImage(name: CtxRefer,
        image: CanvasImageSource | string,
     ...
     */
    drawTip(text: string, id?: string, frame?: number) {
    ...
    drawText(contents, callback) {
        if (contents != null)
            return this._drawText_setContent(contents, callback);
    ...
    _uievent_drawSelector(data) {
        if (data.image == null)
            this.clearUIEventSelector(data.code || 0);
        else
            this.drawUIEventSelector(data.code, data.image, core.calValue(data.x), core.calValue(data.y), core.calValue(data.width), core.calValue(data.height));
    }
    ////// 清除自绘的选择光标
    ...
    drawTextContent(ctx, content, config) {
        ...
        if (config.bold == null)
            config.bold = textAttribute.bold;
        config.italic = false;
    ...
    _drawTextContent_draw(ctx, tempCtx, content, config) {
        ...
        var _drawNext = function () {
            if (config.index >= config.blocks.length)
                return false;
    ...
    _drawTextContent_drawChar(tempCtx, content, config, ch) {
        ...
        if (ch == '\\') {
            ...
            if (c == 'c')
                return this._drawTextContent_changeFontSize(tempCtx, content, config);
            if (c == 'd' || c == 'e') {
                config.index++;
                if (c == 'd')
    ...
    drawTextBox(content: string, config = {}) {
        ...
        if (!posInfo.position)
            posInfo.position = textAttribute.position;
    ...
    _drawTextBox_drawImages(content, ctx) {
        ...
        return content.replace(/(\f|\\f)\[(.*?)]/g, function (text, sympol, str) {
            ...
            if (ss.length == 3)
                ...
                if (ss.length >= 10)
                    core.setAlpha(ctx, parseFloat(ss[9]));
    ...
    _drawTextBox_getHorizontalPosition(content, titleInfo, posInfo) {
        ...
        if (titleInfo.icon != null)
            ...
            paddingLeft = 90; // 10 + 70 + 10
        var left = 7 + 3 * (this.HSIZE - 6), right = this.PIXEL - left, width = right - left, validWidth = width - paddingLeft - paddingRight;
        // 对话框效果：改为动态计算
        if ((posInfo.px != null && posInfo.py != null) || posInfo.pos) {
            var min_width = 220 - paddingLeft, max_width = validWidth;
            // 无行走图或头像，则可以适当缩小min_width
            if (titleInfo.image == null)
                min_width = 160;
            if (titleInfo.title) {
    ...
    _drawTextBox_getVerticalPosition(content, titleInfo, posInfo, validWidth) {
        ...
        if (titleInfo.title)
            height += textAttribute.titlefont + 5;
        if (titleInfo.icon != null) {
            if (titleInfo.title)
                height = Math.max(height, titleInfo.height + 50);
            else
                height = Math.max(height, titleInfo.height + 30);
        }
        else if (titleInfo.image)
    ...
    _drawChoices_drawChoices(choices, isWindowSkin, hPos, vPos, ctx) {
        ...
        for (var i = 0; i < choices.length; i++) {
            ...
            if (main.mode == 'play' && choices[i].need != null && choices[i].need != '' && !core.calValue(choices[i].need))
                color = '#999999';
    ...
    drawConfirmBox(text, yesCallback, noCallback, ctx) {
        ...
        if (core.status.event.selection != 0 && core.status.event.selection != 'none')
    ...
    _drawBook_drawRow2(index, enemy, top, left, width, position) {
        ...
        if (core.flags.statusBarItems.indexOf('enableMoney') >= 0)
            second_line.push([core.getStatusLabel('money'), core.formatBigNumber(enemy.money || 0)]);
        if (core.flags.enableAddPoint)
            second_line.push([core.getStatusLabel('point'), core.formatBigNumber(enemy.point || 0)]);
        if (core.flags.statusBarItems.indexOf('enableExp') >= 0)
            second_line.push([core.getStatusLabel('exp'), core.formatBigNumber(enemy.exp || 0)]);
    ...
    _drawBookDetail_getInfo(index) {
        ...
        if (enemy.description)
            texts.push(enemy.description + "\r");
        texts.push("");
    ...
    _drawBookDetail_origin(enemy, texts) {
        ...
        ["hp", "atk", "def", "point", "money", "exp"].forEach(function (one) {
            if (enemy[one] == null || originEnemy[one] == null)
                return;
    ...
    _drawCenterFly() {
        ...
        var offsetX = core.clamp(toX - core.__HALF_SIZE__, 0, core.bigmap.width - core.__SIZE__), offsetY = core.clamp(toY - core.__HALF_SIZE__, 0, core.bigmap.height - core.__SIZE__);
    ...
    _drawViewMaps(index, x, y) {
        ...
        this.clearUI();
        if (index == null)
            return this._drawViewMaps_drawHint();
        core.animateFrame.tip = null;
        core.status.checkBlock.cache = {};
    ...
    _drawToolbox_getInfo(index) {
        ...
        if (index < this.LAST) {
            ...
            if (select >= tools.length)
                select = Math.max(0, tools.length - 1);
    ...
    _drawEquipbox_getInfo(index) {
        ...
        if (!core.status.hero.equipment)
            core.status.hero.equipment = [];
        ...
        // 处理index
        if (index == null) {
            if (equipLength > 0 && equipEquipment[0])
                index = 0;
            else if (ownEquipment.length > 0)
                index = this.LAST;
            else
                index = 0;
        }
        if (index >= this.LAST && ownEquipment.length == 0)
            index = 0;
        var selectId = null;
        if (index < this.LAST) {
            if (index >= equipLength)
                index = Math.max(0, equipLength - 1);
            selectId = equipEquipment[index] || null;
        }
        else {
            if (page == totalPage)
                index = Math.min(index, (ownEquipment.length + this.LAST - 1) % this.LAST + this.LAST);
    ...
    _drawEquipbox_description(info, max_height) {
        ...
        if (typeof equipType === 'string') {
            ...
            equipString = info.allEquips[equipType] || "未知部位";
    ...
    _drawSLPanel(index, refresh) {
        ...
        if (refresh || page != last_page) {
            ...
            this._drawSLPanel_draw(page, max_page);
    }
    ...
    _drawSLPanel_drawRecord(title, data, x, y, size, cho, highLight) {
        ...
        if (data && data.floorId) {
            ...
            if (core.calWidth('ui', v + v2, this._buildFont(10, false)) <= size)
                v += v2;
    ...
    _drawStatistics_buildObj() {
        ...
        ids.forEach(function (e) {
            if (e.endsWith("Door"))
                ...
                cls[e] = core.material.items[e].cls;
            cnt[e] = 0;
        });
        ...
        ids.sort(function (a, b) {
            ...
            if (c1 == c2)
                return ori.indexOf(a) - ori.indexOf(b);
            return c1 - c2;
        });
        var obj = {
            'monster': {
    ...
    _drawStatistics_floorId(floorId, obj) {
        ...
        // 隐藏层不给看
        if (floor.cannotViewMap && floorId != core.status.floorId)
            return;
    ...
    _drawStatistics_items(floorId, floor, id, obj) {
        ...
        if (id.indexOf('sword') == 0 || id.indexOf('shield') == 0 || obj.cls[id] == 'equips') {
            var t = "";
            if (atk > 0)
                t += atk + core.getStatusLabel('atk');
            if (def > 0)
                t += def + core.getStatusLabel('def');
            if (mdef > 0)
                t += mdef + core.getStatusLabel('mdef');
            if (t != "")
                obj.ext[id] = t;
        }
