...
export class Events {
    ...
    lose(reason) {
        if (core.isReplaying())
            return core.control._replay_error(reason, function () { core.lose(reason); });
        core.status.gameOver = true;
    ...
    doSystemEvent(type, data, callback) {
        ...
        if (this["_sys_" + type])
            return this["_sys_" + type](data, callback);
    ...
    _trigger_inAction(x, y) {
        if (core.status.gameOver || core.status.event.id != 'action')
            return;
    ...
    getItem(id, num, x, y, isGentleClick, callback) {
        ...
        // --- 首次获得道具的提示
        if (!core.hasFlag("__itemHint__"))
            core.setFlag("__itemHint__", []);
    ...
    ////// 获得面前的物品（轻按） //////
    getNextItem(noRoute) {
        if (core.isMoving() || !core.flags.enableGentleClick)
            return false;
    ...
    _canGetNextItem(direction) {
        ...
        if (!core.canMoveHero(null, null, direction))
            return;
    ...
    _changeFloor_getInfo(floorId: string, stair: string, heroLoc, time) {
        ...
        if (main.mode != 'play' || core.isReplaying())
            time = 0;
        if (time == null)
            time = core.values.floorChangeTime;
        time /= 20;
    ...
    ////// 推箱子 //////
    pushBox(data) {
        if (data.event.id != 'box' && data.event.id != 'boxed')
            return;
    ...
    setEvents(list, x, y, callback) {
        ...
        if (!data.appendingEvents)
            data.appendingEvents = [];
        if (!data.locStack)
            data.locStack = [];
    ...
    doAction() {
        ...
        if (core.status.gameOver || core.status.replay.failed)
            return;
        // 判定是否执行完毕
        if (this._doAction_finishEvents())
            return;
        core.clearUI();
        ...
        if (this._popEvents(current, prefix))
            return;
        // 当前要执行的事件
     ...
     */
    insertAction(action: string | MotaAction | MotaAction[], x?: number, y?: number, callback?: () => void, addToLast?: boolean) {
        if (core.hasFlag("__statistics__"))
            return;
        if (core.status.gameOver)
            return;
        if (!action)
    ...
    checkAutoEvents() {
        // 只有在无操作或事件流中才能执行自动事件！
        if (!core.isPlaying() || (core.status.lockControl && core.status.event.id != 'action'))
            return;
        if (core.hasFlag('__doNotCheckAutoEvents__'))
            return;
        var todo = [], delay = [];
        core.status.autoEvents.forEach(function (autoEvent) {
            var symbol = autoEvent.symbol, x = autoEvent.x, y = autoEvent.y, floorId = autoEvent.floorId;
            // 不在当前楼层 or 已经执行过 or 已被分区 or 正在执行中
            if (autoEvent.currentFloor && floorId != core.status.floorId)
                return;
            if (!autoEvent.multiExecute && core.autoEventExecuted(symbol))
                return;
            if ((flags.__removed__ || []).indexOf(floorId) >= 0)
                return;
            if (core.autoEventExecuting(symbol))
                return;
            var prefix = floorId + "@" + x + "@" + y;
            try {
                if (!core.calValue(autoEvent.condition, prefix))
                    return;
            } catch (e) {
                return;
            }
        ...
        if (todo.length == 0 && delay.length == 0)
            return;
    ...
    _action_text(data, x, y, prefix) {
        if (this.__action_checkReplaying())
            return;
    ...
    _action_insert(data, x, y, prefix) {
        if (data.name) { // 公共事件
            ...
            if (event)
                this.insertAction(event.data || event);
        }
        core.doAction();
    }
    ...
    _action_choices(data, x, y, prefix) {
        data.choices = data.choices.filter(function (x) {
            if (x._disabled)
                return false;
            if (x.condition == null || x.condition == '')
                return true;
    ...
    __action_choices_replaying(data, index) {
        ...
        if (index != 'none') {
            ...
            if (isNaN(selection))
                return false;
            if (selection < 0)
                selection += data.choices.length;
            if (selection < 0)
                return false;
            if (selection % 100 > 50)
                selection += data.choices.length;
            if (selection % 100 > data.choices.length)
                return false;
    ...
    _precompile_wait(data) {
        if (data.data) {
            data.data.forEach(function (v) {
                if (v.px != null)
                    v.px = this.__precompile_array(v.px);
                if (v.py != null)
                    v.py = this.__precompile_array(v.py);
                if (v.condition != null)
    ...
    _checkStatus(name, fromUserAction, checkItem) {
        ...
        if (fromUserAction && core.status.lockControl)
            return false;
    ...
    openBook(fromUserAction) {
        ...
        if (!this._checkStatus('book', fromUserAction, true))
            return;
        core.playSound('打开界面');
    ...
    openEquipbox(fromUserAction) {
        if (core.isReplaying())
            return;
        if (!this._checkStatus('equipbox', fromUserAction))
            return;
        core.playSound('打开界面');
    ...
    openToolbox(fromUserAction) {
        if (core.isReplaying())
            return;
        if (!this._checkStatus('toolbox', fromUserAction))
            return;
        core.playSound('打开界面');
    ...
    openQuickShop(fromUserAction) {
        ...
        if (!this._checkStatus('selectShop', fromUserAction))
            return;
    ...
    openKeyBoard(fromUserAction) {
        if (core.isReplaying())
            return;
        if (!this._checkStatus('keyBoard', fromUserAction))
            return;
    ...
    save(fromUserAction) {
        ...
        if (core.status.event.id == 'save' && core.events.recoverEvents(core.status.event.interval))
            return;
        if (!this._checkStatus('save', fromUserAction))
            return;
    ...
    load(fromUserAction) {
        ...
        if (core.status.event.id == 'load' && core.events.recoverEvents(core.status.event.interval))
            return;
        if (!this._checkStatus('load', fromUserAction))
            return;
        core.playSound('打开界面');
        core.ui._drawSLPanel(10 * page + offset);
    }
    ...
    _setValue_setItem(name, value) {
        ...
        if (value > count)
            core.getItem(itemId, value - count);
        else
    ...
    setNameMap(name, value) {
        if (!core.hasFlag('__nameMap__'))
            core.setFlag('__nameMap__', {});
    ...
    setHeroIcon(name, noDraw) {
        ...
        if (core.material.images.hero == img)
            return;
