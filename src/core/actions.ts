...
type EventType
    = "onmove" | "onup" | "ondown" | "onclick"
    | "onmousewheel"
    | "onkeyDown" | "onkeyUp" | "pressKey" | "keyDown" | "keyUp"
    ;
...
export class Actions {
    ...
    actions: Partial<Record<EventType, EventItem[]>> = {};
     ...
     */
    registerAction(action: EventType, name: string, func: () => any, priority = 0) {
        if (!name || !func)
            return;
        // 将onclick视为ondown处理
        if (action == 'onclick')
        ...
        this.actions[action] = this.actions[action]!.sort((a, b) => {
            return b.priority - a.priority;
        });
    }
    ////// 注销一个用户交互行为 //////
    unregisterAction(action: EventType, name: string) {
        // 将onclick视为ondown处理
        if (action == 'onclick')
            action = 'ondown';
        if (!this.actions[action])
            return;
        this.actions[action] = this.actions[action]!.filter((x) => {
            return x.name != name;
        });
    }
    ////// 执行一个用户交互行为 //////
    doRegisteredAction(action: EventType, ...args: any[]) {
        ...
        for (const action of actions) {
            try {
                if (action.func(...args)) return true;
            } catch (e) {
                console.error(`ERROR in action ["${ action.name }"].`, e);
            }
        }
        return false;
    }
    ...
    _sys_keyUp_replay(keyCode, altKey, fromReplay) {
        if (!fromReplay && this._checkReplaying())
            return true;
    }
    ...
    _sys_onmousewheel(direct) {
        ...
        if (core.status.lockControl && core.status.event.id == 'viewMaps') {
            if (direct == 1)
                this._clickViewMaps(this.HSIZE, this.HSIZE - 3, core.__PIXELS__ / 2, core.__PIXELS__ / 5 * 1.5);
            if (direct == -1)
                this._clickViewMaps(this.HSIZE, this.HSIZE + 3, core.__PIXELS__ / 2, core.__PIXELS__ / 5 * 3.5);
            return;
        }
    ...
    _keyDownViewMaps(keycode) {
        ...
        if (keycode == 40 || keycode == 34)
            this._clickViewMaps(this.HSIZE, this.HSIZE + 3, core.__PIXELS__ / 2, core.__PIXELS__ / 5 * 3.5);
        if (keycode == 87 && mh > this.SIZE)
            this._clickViewMaps(this.HSIZE, 0, core.__PIXELS__ / 2, 1);
        if (keycode == 65)
            this._clickViewMaps(0, this.HSIZE, 1, core.__PIXELS__ / 2);
        if (keycode == 83 && mh > this.SIZE)
            this._clickViewMaps(this.HSIZE, this.LAST, core.__PIXELS__ / 2, core.__PIXELS__ - 1);
        if (keycode == 68)
            this._clickViewMaps(this.LAST, this.HSIZE, core.__PIXELS__, core.__PIXELS__ / 2 - 1);
        return;
    }
    ////// 查看地图界面时，放开某个键的操作 //////
    ...
    _keyDownToolbox(keycode) {
        ...
        if (keycode == 38) { // up
            if (index >= this.LAST && index < this.LAST + this.HSIZE) { // 进入tools
                if (toolsTotalPage == 0)
                    return;
                if (toolsLastIndex >= this.HSIZE)
                    index = Math.min(toolsLastIndex, index - this.HSIZE);
                else
                    index = Math.min(toolsLastIndex, index - this.LAST);
            }
            else if (index < this.HSIZE)
                return; // 第一行没有向上
            else
                index -= this.HSIZE;
            this._clickToolboxIndex(index);
        ...
        if (keycode == 40) { // down
            ...
            if (index < this.HSIZE) {
                if (toolsLastIndex >= this.HSIZE)
                    nextIndex = Math.min(toolsLastIndex, index + this.HSIZE);
                else
                    index += this.HSIZE;
            }
        ...
        if (keycode == 38) { // up
            if (index < per_line)
                ...
                if (equipCapacity > per_line)
                    ...
                    index = Math.min(equipCapacity - 1, index);
            }
            else
                index -= this.HSIZE;
            this._clickEquipboxIndex(index);
