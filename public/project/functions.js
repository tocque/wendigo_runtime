var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	// 默认播放的动画；你也可以使用
	var animate = '2'; // 默认动画
	var sound = "attack" // 默认音效
	// 检查当前装备是否存在攻击动画
	var equipId = core.getEquip(0);
	// if (enemyId == '...') animate = '...';

	// 检查是否存在技能，若不存在技能则播放角色对应音效p_skill_jueying_2.wav
	if (core.getFlag("hero") == 4) {
		sound = "p_skill_jueying_2.wav";
		animate = "hand"
	}
	if (core.getFlag('skill') == 1) {
		core.playSound("ks1.wav");
		sound = "Mon3tr1.wav";
	}
	if (core.getFlag('skill') == 4) {
		animate = "fire"
		sound = "Fire1.ogg";
	}
	if (core.getFlag('skill') == 5) {
	...
	//获得源石技艺值
	core.status.hero.mana += (core.getFlag('gr')) * 2;
	if (core.getFlag('hero') == 4) { core.status.hero.mana += (core.getFlag('gr')) * 2 } //塔子姐双倍

	var hint = "打败 " + enemy.name;
	if (core.flags.statusBarItems.indexOf('enableMoney') >= 0) hint += "，金币+" + money;
	if (core.flags.statusBarItems.indexOf('enableExp') >= 0) hint += "，感染值+" + exp;
	...
	if (core.flags.statusBarItems.indexOf('enableSkill') >= 0) {
		...
		if (skill == 1) { // 技能1
			core.status.hero.mana -= 10; // 扣除10点魔力值
		}
		if (skill == 4) { // 技能4
			core.status.hero.mana -= 15; // 扣除15点魔力值
		}
		if (skill == 5) { // 技能5
			core.status.hero.mana -= 25; // 扣除25点魔力值
		}
		// 关闭技能
	...
	*/
	...
	if (itemId == "redGem") core.playSound('atk.wav');
	else if (itemId == "blueGem") core.playSound('def.wav');
	else if (itemId == "greenGem") core.playSound('mdef.wav');
	else if (itemId == "yellowGem") core.playSound('mana.wav');
	else if (itemId == "redPotion" || itemId == "bluePotion" || itemId == "yellowPotion" || itemId == "greenPotion") core.playSound('heal.wav');
	else core.playSound('prts.wav');
	...
	return [
		[1, "先攻", "怪物首先攻击", "#ffcc33"],
		[2, "法术伤害", "攻击无视敌人的防御", "#bbb0ff"],
		[3, "源石技艺·磐石", "该敌人每次最多受到1点伤害", "#b9822d"],
		[4, "2连击", "怪物每回合攻击2次", "#ffee77"],
		[5, "3连击", "怪物每回合攻击3次", "#ffee77"],
		[6, function (enemy) { return "源石技艺·疾风" + enemy.n + "型" }, function (enemy) { return "使自己的身法变得十分敏捷，每回合攻击" + enemy.n + "次" }, "#00FA9A"],
		[7, "源石技艺·剥壳", function (enemy) { return "战斗前，怪物附加角色防御的" + Math.floor(100 * (enemy.defValue || core.values.breakArmor || 0)) + "%作为伤害"; }, "#b30000"],
		[8, "乌萨斯军体拳", function (enemy) { return "战斗时，敌人每回合附加主角攻击的" + Math.floor(100 * (enemy.atkValue || core.values.counterAttack || 0)) + "%作为伤害，无视主角防御"; }, "#ffaa44"],
		[9, "源石技艺·碎盾", function (enemy) { return "战斗时炸开敌人的护盾，造成" + (enemy.n || core.values.purify) + "倍护盾值的伤害"; }, "#00d2d4"],
		[10, "模仿", "怪物的攻防和勇士攻防相等", "#b0c0dd"],
		[11, "源石技艺·汲取", function (enemy) { return "战斗前，敌人首先吸取角色的" + Math.floor(100 * enemy.value || 0) + "%生命（约" + Math.floor((enemy.value || 0) * core.getStatus('hp')) + "点）作为伤害" + (enemy.add ? "，并把伤害数值加到自身生命上" : ""); }, "#ff00d2"],
		...
		[25, "战场指挥", function (enemy) { return "对于一支军队来讲，指挥者永远是核心人物。所有友方单位生命增加" + (enemy.value || 0) / 100 + "倍，攻击增加" + (enemy.atkValue || 0) / 100 + "倍，防御增加" + (enemy.defValue || 0) / 100 + "倍，" + (enemy.add ? "可叠加" : "不可叠加"); }, "#e6e099", 1],
		[26, "支援", "当周围一圈的怪物受到攻击时将上前支援，并组成小队战斗。", "#77c0b6", 1],
		[27, "捕捉", "当走到怪物周围十字时会强制进行战斗。", "#c0ddbb"],
		[28, "反源石镀层", "免疫一切法术伤害", "#B8860B"],
		[29, "零度寒流", "向以自身为中心3格范围的所有敌方单位释放寒流，降低其20%的攻击力和防御力,不可叠加", "#98F5FF", 1],
		[30, "零度寒流+", "向以自身为中心4格范围的所有敌方单位释放寒流，降低其25%的攻击力", "#98F5FF", 1],
		[31, "断杖·冰冻吐息", "寒流如约而至。战斗开始前为敌方施加“冰霜”效果，降低其一半的攻击速度", "#00C5CD"],
		[32, "断杖·内部破坏", "施法者的法术难道不是最好的施法素材吗？战斗时，每回合对敌人造成等同于其法术伤害两倍的法术伤害。", "#EEB422"]
	];
},
	...
	//断杖·内部破坏 暂定为每级感染值增加50%攻击力
	if (core.hasSpecial(mon_special, 32))
		mon_atk = mon_atk * (1 + 0.5 * core.getFlag('gr', 0));
		...
		if (!cache) {
			...
			core.status.maps[floorId].blocks.forEach(function (block) {
				if (!block.disable) {
					...
					// 检查【光环】技能，数字25
					if (enemy && (core.hasSpecial(enemy.special, 25) || core.hasSpecial(enemy.special, 29))) {
						// 检查是否是范围光环
	...
	// 技能的处理
	if (core.getFlag('skill', 0) == 1) { // 开启了技能1：重构
		hero_atk *= 1.2
		hero_def *= 1.4
		hero_mdef *= 1.2; // 算上了Mon3tr和技能本身的加成	
	}
	if (core.getFlag('skill', 0) == 5) { // 开启了技能5：戮刃
		if (core.getFlag('t2', 0) == 0) {
			hero_atk *= 1.3
			mon_def *= 0.5
		}
		if (core.getFlag('t2', 0) == 1) {
			hero_atk *= 1.7
			mon_def *= 0.2
		}
	}
	...
	if ((core.getFlag('skill', 0) == 4) && !(core.hasSpecial(mon_special, 28))) { // 开启了技能4：德火
		if (core.getFlag('t1', 0) == 0) hero_per_damage += mon_atk * 0.5;
		if (core.getFlag('t1', 0) == 1) hero_per_damage += mon_atk * 0.75; //这是根据等级有不同加成,火焰属于真伤	
	}
	if (core.getFlag('skill', 0) == 5) { // 开启了技能5：戮刃
		if (core.getFlag('t2', 0) == 0) hero_per_damage += mon_atk * 0.1;
	}
	if ((core.getFlag('hero', 0) == 4) && !(core.hasSpecial(mon_special, 28))) {
		hero_per_damage += hero_atk * 0.1;
	}
	// 如果没有破防，则不可战斗
	...
	return {
		...
		"damage": Math.floor(damage) * (core.getFlag('gr', 0)) * 0.1 + Math.floor(damage) //gr是感染等级
	};
}
    },
    "actions": {
	...
	case 84: // T：打开道具栏
		...
		break;
	case 81: // Q：打开人物界面
		core.status.route.push("key:81"); // 将按键记在录像中
		core.useItem('wand', true);
		core.useItem('coin', true);
		core.useItem("fly", true);
		core.useItem("centerFly", true);
		break;
	case 90: // Z：转向
		core.turnHero();
		break;
	...
	case 49: // 快捷键1: 角色一技能
		if (core.hasItem('skill1')) {
			...
			core.useItem("skill1", true); // 第二个参数true代表该次使用道具是被按键触发的，使用过程不计入录像
		}
		if (core.hasItem("lifeWand")) {
			core.status.route.push("key:49"); // 将按键记在录像中
			core.useItem("lifeWand", true); // 第二个参数true代表该次使用道具是被按键触发的，使用过程不计入录像
		}
		break;
	case 50: // 快捷键2: 角色二技能
		if (core.hasItem('I324')) {
			core.status.route.push("key:50"); // 将按键记在录像中
			core.useItem("I324", true); // 第二个参数true代表该次使用道具是被按键触发的，使用过程不计入录像
		}
		if (core.hasItem("")) {
			core.status.route.push("key:50"); // 将按键记在录像中
	...
	if (py >= 348 && py <= 377) {
		if (px >= 10 && px <= 39) { core.openBook(true); }
		if (px >= 49 && px <= 78) {
			core.useItem('wand', true)
			core.useItem('coin', true);
			core.status.route.push("key:81");
		}
		if (px >= 89 && px <= 118) { core.openToolbox(true); }
	}
	if (py >= 385 && py <= 414) {
		if (px >= 9 && px <= 38) { core.save(true); }
		if (px >= 49 && px <= 78) { core.load(true); }
		if (px >= 89 && px <= 118) { core.openSettings(true); }
	}
	...
	if (core.domStyle.isVertical) {
		core.statusBar.hard.innerText = "skill"
	}
	...
	*/
	...
	var _fillBoldTextWithFontCheck = function (text, x, y, style) {
		...
		if (!/^[-a-zA-Z0-9`~!@#$%^&*()_=+\[{\]}\\|;:'",<.>\/?]*$/.test(text))
			core.setFont(ctx, 'bold 18px black');
		else core.setFont(ctx, 'bold 18px black');
		core.fillText(ctx, text, x, y, style);
	}
	...
	if (!core.domStyle.isVertical) {
		// 横屏模式
		// 绘制背景图层
		if (core.getFlag('hero', 0) == 0) {
			core.drawImage(ctx, ('ktshuping.png'), 0, 0, 129, 416)
		}
		if (core.getFlag('hero', 0) == 4) {
			core.drawImage(ctx, ('tllshuping.png'), 0, 0, 129, 416)
		}
		// 绘制感染等级
		core.drawTextContent(ctx, "感染等级" + core.formatBigNumber(core.getFlag('gr', 0)), { left: 33, top: 308, font: "black", fontSize: 13, bold: true, color: "#FF3030" });
		if (core.getFlag('gr', 0) != 0) {
			if (core.getFlag('hero', 0) == 4) {
				core.drawTextContent(ctx, "技艺值恢复" + core.formatBigNumber(core.getFlag('gr', 0) * 4), { left: 40, top: 30, font: "black", fontSize: 15, bold: true, color: "#1E90FF" });
			} else {
				core.drawTextContent(ctx, "技艺值恢复" + core.formatBigNumber(core.getFlag('gr', 0) * 2), { left: 40, top: 30, font: "black", fontSize: 15, bold: true, color: "#1E90FF" });
			}
		}
		/*绘制魔力值
		（为了美观，这俩是一起的）
		...
		if (core.getFlag('skill', 0) == 0) {
			core.drawTextContent(ctx, '无', { left: 0, top: 50, font: "black", fontSize: 15, bold: true, color: "#FFD700" })
			_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mana')), 0, 45, '#1E90FF');
		}
		if (core.getFlag('skill', 0) == 1) {
			core.drawTextContent(ctx, '指令：结构重组', { left: 26, top: 50, font: "black", fontSize: 15, bold: true, color: "#7CFC00" })
			_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mana')), 0, 45, '#1E90FF')
			core.drawIcon(ctx, "skill1", 0, 46, 25, 25);
		}
		if (core.getFlag('skill', 0) == 4) {
			core.drawTextContent(ctx, '德拉克之火', { left: 26, top: 50, font: "black", fontSize: 15, bold: true, color: "#FF3030" })
			_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mana')), 0, 45, '#1E90FF')
			core.drawIcon(ctx, "lifeWand", 0, 46, 25, 25);
		}
		if (core.getFlag('skill', 0) == 5) {
			core.drawTextContent(ctx, '日冕戮刃', { left: 26, top: 50, font: "black", fontSize: 15, bold: true, color: "#FF3030" })
			_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mana')), 0, 45, '#1E90FF')
			core.drawIcon(ctx, "I324", 0, 46, 25, 25);
		}
		// 绘制生命
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('hp')), 33, 174, '#363636');

		// 绘制攻击
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('atk')), 33, 209, '#363636');

		// 绘制防御
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('def')), 33, 244, '#363636');

		// 绘制护盾
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mdef')), 33, 279, '#363636');

		// 绘制经验
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.status.hero.exp) + "/100", 34, 308, '#363636');

		// 绘制三色钥匙
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('yellowKey')), 45, 345, '#FFCCAA');
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('blueKey')), 99, 345, '#AAAADD');

	} else {
		// 竖屏模式
		//绘制天杀的背景
		if (core.getFlag('hero', 0) == 0) {
			core.drawImage(ctx, ('kth.png'), 0, 0, 416, 105)
		}
		if (core.getFlag('hero', 0) == 4) {
			core.drawImage(ctx, ('tth.png'), 0, 0, 416, 105)
		}
		// 绘制技能
		if (core.getFlag('skill', 0) == 0) {
			core.drawTextContent(ctx, '无', { left: 142, top: 88, font: "black", fontSize: 15, bold: true, color: "#FFD700" })
		}
		if (core.getFlag('skill', 0) == 1) {
			core.drawTextContent(ctx, '指令：结构重组', { left: 142, top: 88, font: "black", fontSize: 15, bold: true, color: "#7CFC00" })
		}
		if (core.getFlag('skill', 0) == 4) {
			core.drawTextContent(ctx, '德拉克之火', { left: 142, top: 88, font: "black", fontSize: 15, bold: true, color: "#FF3030" })
		}
		if (core.getFlag('skill', 0) == 5) {
			core.drawTextContent(ctx, '日冕戮刃', { left: 142, top: 88, font: "black", fontSize: 15, bold: true, color: "#FF3030" })
		}
		// 绘制生命
		...
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('def')), 39, 60);

		// 绘制护盾

		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mdef')), 173, 60);

		// 绘制经验

		_fillBoldTextWithFontCheck(core.formatBigNumber(core.status.hero.exp) + "/100", 39, 87);
		//绘制感染等级
		core.drawTextContent(ctx, core.formatBigNumber(core.getFlag('gr', 0)), { left: 39, top: 86, font: "black", fontSize: 15, bold: true, color: "#FF3030" });
		//绘制魔力值
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('mana')), 103, 26, '#1E90FF');
		// 绘制三色钥匙
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('yellowKey')), 142, 86, '#FFCCAA');
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('blueKey')), 177, 86, '#AAAADD');
		_fillBoldTextWithFontCheck(core.setTwoDigits(core.itemCount('redKey')), 212, 86, '#FF8888');
	}
},
